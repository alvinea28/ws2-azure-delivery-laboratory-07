...
Zero/skipped tests are not passes. Lab 07 offline study needs no prior lab, but this private [scoped live route](delivery-configuration.md) requires approved identities, backend, restricted runner and encrypted exact plans, not a manual deployment reviewer. Main-only environments have no Required reviewers and no admin bypass. Scope/budget/bootstrap authorization and dedicated admin-authorized cleanup remain separate; ordinary main never cleans up. Never start/rerun live work to repair setup; issue progress grants no Azure permission.
