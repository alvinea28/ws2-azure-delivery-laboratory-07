...
| PR → Files changed / Checks | Author inspection, strict checks and resolved conversations; [zero source-PR approvals](pr-author-merge.md) | Once ready, protected main automatically applies the exact saved plan, no manual deployment reviewer; retain scope/bootstrap/cleanup authorizations; template maintenance dev |
