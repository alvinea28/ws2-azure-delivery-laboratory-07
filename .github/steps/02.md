...
Do not select **Run workflow** for deployment and never select **Re-run jobs**. Only **followup** remains in delivery's manual menu; [dedicated cleanup](05.md) accepts a required authorization string, no operation input. Scheduled drift reports without applying, but needs the owner's deliberate protected-main default choice when ready; the offline default dev is not changed automatically. Read [Step 3's exact-plan observations](03.md#4-observe-scoped-authorization-and-exact-plan-application) before the first enabled merge.
...
**Next:** [Step 3: automatic exact-plan deploy](03.md). Offline success cannot unlock it.
