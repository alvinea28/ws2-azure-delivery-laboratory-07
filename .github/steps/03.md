...
Keep every [Step 2 prerequisite](02.md); public/unapproved copies remain offline. No local apply, state access, reviewer wait or credentials in PR checks.
...
Compare resource IDs with the run and keep observations private. Subnets are nested, not separate RG rows. Green Actions and output IDs are **not actual configuration proof**. Make the single benign update only in [Step 4](04.md).
