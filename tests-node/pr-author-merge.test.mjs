...
test("PR guide explains automatic exact-plan apply and separate cleanup with standalone links", async () => {
  ...
  const suffix = course.number === 2 ? "network-module-laboratory-02" : "azure-delivery-laboratory-07";
  const cleanup = course.number === 2 ? "avm-cleanup.yml" : "cleanup.yml";
  assert.ok(prose.includes(`alvine-aurelio-org/ws2-sim-20260921-${suffix}`));
...
test("PR instructions stop at absent main and missing authorized prerequisites, not fabricated readiness", async () => {
  const doc = await read("docs/pr-author-merge.md");
  const text = doc.replace(/[*`]/g, "").replace(/\s+/g, " ");
  for (const pattern of [/If protected main does not exist.*stop at the offline handoff/i, /Do not create main or open a PR into a nonexistent branch/i, /only after baseline\/readiness review, while delivery is disabled/i, /no new settings or Azure observation/i, /budget\/currency, lifetime and explicit bootstrap authorization/i, /OIDC, state\/leases, runner, encryption keys and module App must be verified/i, /Public source-template maintenance remains on dev, never live main/i, /do not edit or repin the immutable repository allowlist/i, /No live success is claimed/i]) assert.match(text, pattern);
  assert.ok(doc.includes("delivery-configuration.md"), "Readiness belongs to the context-specific configuration record");
  assert.doesNotMatch(text, /Last supplied (?:live )?readback|no main, environments, runners, secrets/i);
  assert.match(text, /reuses the existing pinned ruleset 23780383/);
  assert.match(text, /Do not create or import a replacement ruleset/);
  assert.match(text, /missing or changed, keep delivery disabled.*owner to reconcile/i);
  assert.doesNotMatch(text, /Rulesets → New branch ruleset/);
});

test("public source setup and existing private observations cannot be conflated into PR permission", async () => {
  const config = (await read("docs/delivery-configuration.md")).replace(/[*`]/g, "").replace(/\s+/g, " ");
  if (config.includes("Setup status — public source")) {
    assert.match(config, /expected configuration, not observed private settings/i);
    assert.doesNotMatch(config, /Verified configuration update, \d{2}:\d{2} UTC|dev-plan \(\d{10,}\)|dev-apply \(\d{10,}\)/i);
  } else {
    assert.match(config, /Setup status — existing private observations/);
    assert.match(config, /not a new readback from this lesson edit/i);
    assert.match(config, /later environment update supersedes only the initial absence of environments/i);
  }
  assert.match(config, /Missing\/changed revision fails closed/i);
  assert.match(config, /zero bypass actors/i);
});
